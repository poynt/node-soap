...
WSDL.prototype._initializeOptions = function (options) {
  ...
  // hacks hacks hacks
  this.options.overrideXmlArrayName = options.overrideXmlArrayName;
};
...
WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns) {
  ...
  for (var key in params) {
    if (key !== nsAttrName) {
      ...
      parts.push(['<', key, Array.isArray(value) ? ' enc:arrayType="tns:ArrayOfString[' + value.length + ']" xsi:type="enc:Array"' : (typeof value === 'string' ? ' xsi:type="xsd:string"' : ''), '>'].join(''));
    ...
    for (var i = 0, item; item = obj[i]; i++) {
      ...
      if (!arrayAttr) {
        if (Array.isArray(item)) {
          arrayAttr = ' enc:arrayType="xsd:string[' + item.length + ']" xsi:type="enc:Array"';
        } else if (typeof item === 'string') {
          arrayAttr = ' xsi:type="xsd:string"';
        }
      }

      xmlArrayName = self.options.overrideXmlArrayName || name;

      parts.push(['<', xmlArrayName, arrayAttr, xmlnsAttrib, '>'].join(''));
      parts.push(self.objectToXML(item, name, namespace, xmlns, false, null, parameterTypeObject, ancXmlns));
      parts.push(['</', xmlArrayName, '>'].join(''));
    }
